...
import logger, { LogCategory, LogLevel } from '@/lib/logger.server';
...
class EmailService {
  ...
  async initializeOAuth() {
    try {
      ...
      if (!clientId || !clientSecret || !refreshToken || !userEmail) {
        logger.warning(
          LogCategory.SYSTEM,
          'Email service: OAuth credentials not configured. Email notifications will be disabled.'
        );
        return;
      }
      ...
      logger.info(LogCategory.SYSTEM, 'Email service: OAuth2 configured successfully');
    } catch (error) {
      logger.error(LogCategory.SYSTEM, 'Email service: Failed to initialize OAuth2', {
        error: error.message,
      });
      this.isConfigured = false;
    }
  ...
  async createTransporter() {
    ...
    try {
      ...
      logger.error(LogCategory.SYSTEM, 'Email service: Failed to create transporter', {
        error: error.message,
      });
      throw error;
    }
  ...
  async sendEmail({ to, subject, html, text }) {
    if (!this.isConfigured) {
      logger.warning(
        LogCategory.SYSTEM,
        'Email service: Cannot send email - service not configured',
        { to, subject }
      );
    ...
    try {
      ...
      logger.info(LogCategory.SYSTEM, 'Email sent successfully', {
        to,
        subject,
      ...
      logger.error(LogCategory.SYSTEM, 'Email service: Failed to send email', {
        to,
        subject,
        error: error.message,
      });
  ...
  async sendTestEmail(to) {
    ...
    if (result.success) {
      logger.info(LogCategory.SYSTEM, 'Test email sent successfully', { to });
    } else {
      logger.error(LogCategory.SYSTEM, 'Failed to send test email', { to, error: result.message });
    }
    return result;
  }
  ...
  async sendCloneSuccessNotification(to, jobDetails) {
    ...
    const result = await this.sendEmail({ to, subject, html });
    if (result.success) {
      logger.success(LogCategory.CLONE_OPERATION, `Clone success email sent for job: ${jobDetails.jobName}`, { to, jobName: jobDetails.jobName });
    } else {
      logger.error(LogCategory.CLONE_OPERATION, `Failed to send clone success email for job: ${jobDetails.jobName}`, { to, jobName: jobDetails.jobName, error: result.message });
    }
    return result;
  }
  ...
  async sendCloneErrorNotification(to, jobDetails, error) {
    ...
    const result = await this.sendEmail({ to, subject, html });
    if (result.success) {
      logger.info(LogCategory.CLONE_OPERATION, `Clone error email sent for job: ${jobDetails.jobName}`, { to, jobName: jobDetails.jobName });
    } else {
      logger.error(LogCategory.CLONE_OPERATION, `Failed to send clone error email for job: ${jobDetails.jobName}`, { to, jobName: jobDetails.jobName, error: result.message });
    }
    return result;
  }
